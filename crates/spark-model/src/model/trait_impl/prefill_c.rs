...
impl TransformerModel {
    pub(super) fn prefill_twophase_dispatch(
        ...
        for (i, layer) in self.layers.iter().enumerate() {
            ...
            // ATLAS_NEMO_DUMP: write LAST token's hidden (fp32) per layer, FIRST
            // prefill request only (a once-latch prevents decode-prefill overwrite).
            if let Ok(dir) = std::env::var("ATLAS_NEMO_DUMP")
                && !dir.is_empty()
            {
                static DUMP_DONE: std::sync::atomic::AtomicBool =
                    std::sync::atomic::AtomicBool::new(false);
                if !DUMP_DONE.load(std::sync::atomic::Ordering::Relaxed) {
                    self.gpu.synchronize(stream)?;
                    let hsz = self.config.hidden_size;
